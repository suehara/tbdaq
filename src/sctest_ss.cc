...
void slowShaperMulti(int id,int event,unsigned char *sc,const char *filenamebase){
  ...
  for(int i=0;i<8;i++){
    ...
    for(int ch=0; ch<8; ch++){
      writeNumber(10+i*8+ch*3,1,1,sc);
    };
    writeSlowControl(id,sc,false);     


    ostringstream osss8;
    osss8 << filenamebase <<"_F" << i*8 << "T"<< i*8+7 << "_injected.raw";
    
    Acquisition(id,event,osss8.str().c_str()); 

    for(int ch=0; ch<8; ch++){
      writeNumber(10+i*8+ch*3,1,0,sc);
    };
    writeSlowControl(id,sc,false);     
...
int main(int argc,char **argv)
{
  ...
  */
    ...
    //    thresholdScanMulti(id,360,450,4,100,sc,"tmp/thresholdscan_2MIP"); // trigger thresold
       Acquisition(id,500,"tmp/slowshaper.raw");
