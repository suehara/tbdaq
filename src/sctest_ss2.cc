...
void slowShaperMulti(int id,int event,unsigned char *sc,const char *filenamebase){
  ...
  for(int i=0;i<8;i++){
    for(int ch=0; ch<8; ch++){
     ...
     ostringstream osss1;
     osss1 << filenamebase << "_startch"<< 8*i<< ".raw";
...
int main(int argc,char **argv)
{
  try{
    ...
    //    int id = OpenUsbDevice((char *)"SK2_04"); //tb4 skiroc2a, tb1_1
    //    int id = OpenUsbDevice((char *)"SK2_02"); //tb2 skiroc2
  ...
  */
    ...
    const char *file[] ={
      ...
      //      "../skiroc2a_ADC_25.txt" //tb1_5 skiroc2a 4-bit dac off  allch th280
      ...
      //      "../skiroc2_tb2_ADC_21_180.txt"  //tb2 skiroc2 th180 6pF
    };
    ifstream f;
    f.open(file[0]);
    if(f.good())
    ...
    //    slowShaper(id,500,sc,"tmp2/slowshaper");
    //      Acquisition(id,1000,"tmp2/slowshaper.raw");      

    slowShaperMulti(id,100,sc,"tmp2/slowshaper_fC12pF");
