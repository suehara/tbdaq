...
int writeSlowControl(int id, unsigned char sc[], bool probe){
  ...
  int rd,wr;
    ...
    unsigned char *sc3 = new unsigned char[scsize];
    unsigned char k;
    /*  
   ...
   for(i=0; i<77; i++){      
    ...
    UsbWrt(id, 10, &sc, 77);
    cout <<"Slow control parameters read." << endl;
 ...
 //Stop Cycle
    UsbRd(id, 1, &rd, 1);
    wr =rd & 0xFD; //bit1 reverse
    UsbWrt(id, 1, &wr, 1);
    cout << "Stop cycle" << endl;
 
 //SR register correlation test query
    cout << "SR register correlation test start." << endl;
    ...
    UsbWrt(id, 10, &sc, 77);
    cout << "Slow control register finished." <<endl;
    ...
    cout << "SR register test : " << bitset<8>(rd) <<endl;
 
//SR resister correlation test query end
    UsbRd(id, 0, &rd, 1); //word0
    wr = rd & 0x7F; //bit7 reverse
    UsbWrt(id, 0, &wr, 1);
    cout << "SR register correlation test finished." << endl;
...
int main(int argc,char **argv)
{
 ...
 //reset      
    ...
    //    Acquisition(id,100,"test.raw");
    //    delayScan(id,0,255,16,100,sc,"delayscan");
