...
void writeNumberLM(int subadd,int size,int value,unsigned char sc[]){

  int bit,bin;

  bit = ((615-(subadd+size-1))%8);
  bin = (615-(subadd+size-1))/8;
...
void thresholdScan(int id,int start,int stop,int step,int event,unsigned char *sc, const char *filenamebase){
  ...
  for(int i=0;i<8;i++){
    ...
    for(int ch =0;ch < 8;ch++){
      writeNumber(10+3*(8*i+ch),1,1,sc);
    }  //for ch    
    ...
    for(int thresold=start; thresold<= stop; thresold+=step){
      ...
      ostringstream oss;
      oss << filenamebase << "_" << thresold << "_startch"<< 8*i << ".raw" ;
      Acquisition(id,event,oss.str().c_str());
    ...
    for(int ch =0;ch < 8;ch++){
      writeNumber(10+3*(8*i+ch),1,0,sc);
    }  //for ch
...
void th4bit(int id,int start,int stop,int step,int event,unsigned char *sc, const char*filenamebase){

  for(int i=0;i<3;i++){
    writeNumber(10+90*i,1,1,sc);
    writeSlowControl(id,sc,false);
    
    for(int th=0;th<16;th++){
      writeNumberLM(223+120*i,4,th,sc);
      writeSlowControl(id,sc,false);
      ostringstream oss4;
      oss4 << filenamebase << "_ch" << i*30 << "_4bitth" << th;
      thresholdScan(id,start,stop,step,event,sc,oss4.str().c_str());
    }
    writeNumber(10+90*i,1,0,sc);
  }

};


void th4bit64(int id,int start,int stop,int step,int event,unsigned char *sc, const char*filenamebase){

  for(int ch=0;ch<64;ch++){
    writeNumber(10+3*ch,1,1,sc);
    writeSlowControl(id,sc,false);
    
    for(int th=0;th<16;th++){
      writeNumberLM(223+4*ch,4,th,sc);
      writeSlowControl(id,sc,false);
      ostringstream oss64;
      oss64 << filenamebase << "_ch" << ch <<"_4bitth" << (15-th)<< ".raw";
      Acquisition(id,event,oss64.str().c_str());
    }
...
int main(int argc,char **argv)
{
  try{
    ...
    int id = OpenUsbDevice((char *)"SK2_01");   //tb1_1
    //    int id = OpenUsbDevice((char *)"SK2_04"); //tb2 skiroc2a, tb1_1
    //    int id = OpenUsbDevice((char *)"SK2_02"); //tb2 skiroc2, tb1_1
  ...
  */
    ...
    //     "../skiroc2a_tb1_ADC_allchoff.txt" //th210 skiroc2a allch injection off
      //      "../skiroc2_tb2_ADC_20.txt"  //tb2 skiroc2 th280 4-bit
      //      "../skiroc2a_tb2_ADC_21.txt"  //tb2 skiroc2a th300 4-bit
      ...
      //      "../skiroc2a_ADC_25.txt" //tb1_5 skiroc2a 4-bit dac off allch
      //      "../skiroc2a_tb2_ADC_15.txt"  //tb2 skiroc2a
      //      "../skiroc2a_tb2_ADC_24.txt"  //tb2 skiroc2a allch injected

      //      "../skiroc2_tb2_ADC_20.txt"  //tb2 skiroc2 th260 6pF
    ...
    */
    ...
    Acquisition(id,100,filename);
    
    //   delayScan(id,0,256,1,500,sc,"tmp/delayscan");

    //    thresholdScan(id,260,310,10,100,sc,"tmp2/thresholdscan_1MIP"); // trigger thresold


    //    thresholdScanMulti(id,160,210,2,100,sc,"tmp2/thresholdscan_1MIP"); // trigger thresold
    ...
    //    th4bit(id,250,340,4,100,sc,"tmp/thresholdscan_1MIP");
    //    th4bit64(id,250,340,4,100,sc,"tmp/thresholdscan_1MIP");
